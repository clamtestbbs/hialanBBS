     ...
     switch (ch)
      {
       ...
       case Ctrl('Z'):
       {
         ...
         if(re_entry)
         {
           pressanykey("±z¤w¸g¨Ï¥Î§Ö³t¿ï³æ¤F!");
           return ch;
         }         
	   ...
	   char ans='m';
 	   ...
 	   while(ans != 'q')
 	   {
  	     clear();
  	     setutmpmode(QMENU);
             switch(ans = win_select("§Ö³t¿ï³æ", "", choose, 12, ans))
             {
               case 'm':
                 m_read();
                 break;
               ...
               case 'u':
                 if(currutmp->mode != LUSERS)
                   ...
                   pressanykey("±z¤w¸g¦b¨Ï¥ÎªÌ¦W³æ¤F!!");
                 break;
               case 'b':
                 New();
                 break;
